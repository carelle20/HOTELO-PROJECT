...
import { LayoutDashboard, Hotel, BedDouble, CalendarCheck, Users, Star, Wallet, CreditCard, Settings, LogOut } from "lucide-react";
...
const menuSections = [
  {
    ...
    items: [
      ...
      { label: "Chambres", icon: BedDouble, path: "/manager/chambres" },
      { label: "Réservations", icon: CalendarCheck, path: "/manager/reservations" },
      { label: "Clients", icon: Users, path: "/manager/clients" },
      { label: "Avis", icon: Star, path: "/manager/avis" },
    ],
  },
  {
    title: "FINANCES",
    items: [
      { label: "Revenus", icon: Wallet, path: "/manager/revenus" },
      { label: "Paiements", icon: CreditCard, path: "/manager/paiements" },
    ],
  },
];

export default function Sidebar() {
  const { logout } = useAuth(); // Connexion au contexte
  ...
  return (
    <aside className="w-64 bg-white border-r border-gray-200 flex flex-col h-screen sticky top-0">
      ...
      <div className="flex-shrink-0 border-t border-gray-200 p-4 space-y-1">
        <NavLink to="/manager/settings" className="flex items-center gap-3 px-4 py-2 rounded-lg text-gray-600 hover:bg-gray-100">
